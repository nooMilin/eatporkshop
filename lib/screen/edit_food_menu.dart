...
class _EditFoodMenuState extends State<EditFoodMenu> {
  ...
  Future<Null> confirmEdit() async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('คุณต้องการจะ เปลี่ยนแปลงเมนูอาหารจริงๆนะ ?'),
        children: <Widget>[
          Row(
            children: <Widget>[
              FlatButton.icon(
  ...
  Future<Null> editValueOnMySQL() async {
    ...
    String url = "${MyConstant().domain}/eatporkshop/api/editFoodWhereId.php?isAdd=true&id=$id&NameFood=$name&PathImage=$pathImage&Price=$price&Detail=$detail";
    await Dio().get(url).then((value){
      if (value.toString() == 'true') {
        Navigator.pop(context);
      } else {
        normalDialog(context, 'กรูณาลองใหม่ ค่ะ');
      }
    });
