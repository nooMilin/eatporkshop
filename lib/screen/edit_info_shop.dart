...
class _EditInfoShopState extends State<EditInfoShop> {
  ...
  Future<Null> confirmDialog() async {
   ...
   Future<Null> editThread() async {
    ...
    String urlUpload = '${MyConstant().domain}/eatporkshop/api/saveShop.php';
    await Dio().post(urlUpload, data: formData).then((value) async {
      urlPicture = '/eatporkshop/api/Shop/$nameFile';

      String id = userModel.id;

      String url =
          '${MyConstant().domain}/eatporkshop/api/editUserWhereId.php?isAdd=true&id=$id&NameShop=$nameShop&Address=$address&Phone=$phone&UrlPicture=$urlPicture&Lat=$lat&Lng=$lng';
    ...
    return <Marker>[
      Marker(
        ...
        infoWindow: InfoWindow(
            title: 'ร้านอยู่ที่นี่', snippet: 'Lat = $lng, Lng = $lng'),
      )
    ].toSet();
  }
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            Container(
              width: 250.0,
              height: 250.0,
              child: file == null ? Image.network('${MyConstant().domain}$urlPicture') : Image.file(file),
            ),
            IconButton(
